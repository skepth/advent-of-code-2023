...
impl Game {
    // NOTE: This method was written assuming we needed to check the total
    // count of the colors in all the game set, but turns out we only need
    // to ensure each game is below the possible color counts.
    //
    // Doesn't help that the unit test passes with this incorrect assumption.
    // Using this approach only yielded 208 as the answer.
    fn _get_color_totals(&self, color: COLORS) -> u32 {
    ...
    // This method checks if the game set is valid by ensuring if each game set
    // meets the max values threshold.
    fn is_valid(&self) -> bool {
        let want: EnumMap<COLORS, u32> = enum_map!(
            COLORS::RED => 12,
            COLORS::GREEN => 13,
            COLORS::BLUE => 14,
        );
        for game in &self.turns {
            if game[COLORS::RED] > want[COLORS::RED] {
                return false;
            } else if game[COLORS::GREEN] > want[COLORS::GREEN] {
                return false;
            } else if game[COLORS::BLUE] > want[COLORS::BLUE] {
                return false;
            }
        }
        return true;
